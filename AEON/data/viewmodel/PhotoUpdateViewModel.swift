...
//  AEONVCS
//
//  Created by Khin Yadanar Thein on 7/25/19.
...
class PhotoUpdateViewModel {
    
    //MAKE VERIFY MEMBER REGISTER
    func updateProfileImage(customerId:String, profileImage:UIImage, token:String, refreshToken:String, success: @escaping (ProfileUpdateResponse) -> Void,failure: @escaping (String) -> Void){
        ...
        let updateData = try! jsonEncoder.encode(rawData)
        //let verifyData = String(data: jsonData, encoding: String.Encoding.utf8)
        
        PhotoUpdateModel.init().updateImage(rawData:updateData, token: token, success: { (result) in
            ...
            if error == Constants.EXPIRE_TOKEN {
                LoginAuthModel.init().refereshToken(refreshToken: refreshToken, success: { (result) in
                    ...
                    if result.status == Constants.STATUS_200 {
                        ...
                        PhotoUpdateModel.init().updateImage(rawData:updateData,token: token.accessToken!, success: { (result) in
                            if result.status == Constants.STATUS_200 {
                                success(result)
