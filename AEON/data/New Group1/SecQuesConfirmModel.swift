...
class SecQuesConfirmModel: BaseModel {
    ...
    func makeConfirm(userConfirmRequest: UserSecQuesConfirmRequest,success: @escaping (UserSecQuesResponse) -> Void,failure: @escaping (String) -> Void){
        
        let jsonEncoder = JSONEncoder()
        let jsonData = try! jsonEncoder.encode(userConfirmRequest.quesAnsBean)
        let qaString = String(data: jsonData, encoding: String.Encoding.utf8)

        let rawData = [
            "phoneNo" : userConfirmRequest.phoneNo,
            "nrc" : userConfirmRequest.nrcData,
            "qaList" : qaString
        ]
