...
//                print("original image width & height", pickedImage.size.width, pickedImage.size.height)
                ...
                } else if jpegSize > Constants.HIGH_QUALITY_IMAGE_1500 && jpegSize <= Constants.HIGH_QUALITY_IMAGE_3000 {
...
//let socket = WebSocket("wss://ass.aeoncredit.com.mm")

//        self.messagingSocket.event.open = {
//
//            print("message socket opened : \(self.senderName)userId:\(self.senderId)")
//            self.messagingSocket.send("userName:\(self.senderName)userId:\(self.senderId)")
//            self.messagingSocket.send("cr:\(self.senderName)or:userWithAgency:")
//            //self.isAlreadyConnect = true
//
//        }
//        self.messagingSocket.event.close = { code, reason, clean in
//            print("message socket close", reason)
//            let isClose = UserDefaults.standard.bool(forKey: Constants.MESSAGE_SOCKET_CLOSE)
//            if isClose {
//                self.messagingSocket.close()
...
//
//            } else {
//                self.messagingSocket.open()
//            }
//            CustomLoadingView.shared().hideActivityIndicator(uiView: self.view)
//
//        }
//        self.messagingSocket.event.error = { error in
//            print("message socket error \(error)")
//
//            let isClose = UserDefaults.standard.bool(forKey: Constants.MESSAGE_SOCKET_CLOSE)
//            if isClose {
...
//
//        }
//        self.messagingSocket.event.message = { message in
//
//            //print("socket onMessage \(message as? String)")
...
//                                // chat room is open and get message list
//                                if (self.messageBeanList.count == 0){
//                                    self.messagingSocket.send("messageList:")
//
//                                } else {
//                                    self.messagingSocket.send("unReadMessageList:")
//                                }
//
//                            } else if (type == "messageListData"){
//                                //let data = jsonData["data"] as! NSArray
//                                //print("data", data.value(forKey: "messageContent"), data.value(forKey: "opSendFlag"))
//                                if let content = jsonData["data"] as? NSArray {
//                                    print("message list array size :", content.count)
//
//                                    if (content.count > 0){
...
//                                                if 0 == (content[index] as AnyObject).value(forKey: "readFlag") as? Int {
//                                                    if messageMenu == 5 {
//                                                        self.messagingSocket.send("ChangeReadFlagWithMsgId:\(((content[index] as AnyObject).value(forKey: "messageId") as? Int)!)")
//
//                                                    } else {
//                                                    unreadArray.append(((content[index] as AnyObject).value(forKey: "messageId") as? Int)!)
//                                                    }
...
//                                        //check if segment is message
//                                        if messageMenu == 5 {
//                                            self.messagingSocket.send("ChangeReadFlagWithMsgId:\((data.value(forKey: "message_id") as? Int)!)")
//                                        } else {
//                                            unreadArray.append((data.value(forKey: "message_id") as? Int)!)
//                                            UserDefaults.standard.set(unreadArray, forKey: Constants.UNREAD_MESSAGE_ARRAY)
//                                            UserDefaults.standard.set(0, forKey: Constants.UNREAD_MESSAGE_COUNT)
...
//                                self.tvMessagingView.insertRows(at: [indexPath], with: .automatic)
//                                self.tvMessagingView.endUpdates()
//                                self.tvMessagingView.scrollToRow(at: indexPath, at: .none, animated: true)
//                                CustomLoadingView.shared().hideActivityIndicator(uiView: self.view)
//
//                            // clear unread message array
//                            } else if (type == "showMessage") {
